...
const addPerformanceIndexes = async () => {
  try {
    ...
    await sequelize.query(`
      ...
      ON "RefreshTokens"("token") WHERE "isRevoked" = false;
    `);

    // Index sur les tables de relations pour éviter les N+1
    console.log('🛒 Ajout d\'index sur CartItems...');
    await sequelize.query(`
      CREATE INDEX IF NOT EXISTS idx_cart_items_cart_id ON cartitems("cartId");
    `);

    await sequelize.query(`
      CREATE INDEX IF NOT EXISTS idx_cart_items_star_id ON cartitems("starId");
    `);

    console.log('💝 Ajout d\'index sur Wishlist...');
    await sequelize.query(`
      CREATE INDEX IF NOT EXISTS idx_wishlist_user_id ON wishlists("userId");
    `);
    ...
    await sequelize.query(`
      CREATE INDEX IF NOT EXISTS idx_reviews_star_id ON reviews("starId");
    `);

    // Index pour les timestamps (createdAt) pour les tris par date
    console.log('📅 Ajout d\'index sur les timestamps...');
    await sequelize.query(`
      CREATE INDEX IF NOT EXISTS idx_stars_created_at ON stars("createdAt" DESC);
    `);
